...
Given an array return the majority elements.  The majority element would appears more than half length of the array times.

### Solution

The two pass solution is obvious. The first pass would record how many times an element appears with a Map. And the second pass would go over the Map find the element with max appears. This solution is general to find the element with most frequency. However, this does not consider the condition that majority elements occupied more than half or the array. This leads to the one pass solution. Because the majority element is more than the half, the difference in count between it and all the rest elements is greater than 0. So we can visit the elements one by one and if the new elements match the current candidate, then the count would be add 1. If not, it would be minus 1. When the count is less than zero, the current element would become the candidates. Because the majority is more than the half. At the end of the iteration, it would be the candidates left. This is call Boyer-Moore majority vote algorithm.
...
Note. This question can be solved with divide and conquer as well. Split the original array into half. Keep doing it until only one element left. Then we can start merge. Merging would requires go through all the current sub array and find out the majority elements. It works but this would be O(nlogn) so the Boyce-Moore is still the preferred solution.
...
4. each elements are removed at most k times.
5. The majority elements are more than ⌊ n/3 ⌋. They must left in the final result.
...
With above analysis, we can generalize the question as follow.
