...
Best Time to Buy and Sell Stock are a serial problems can be solved by dynamic programming. Although some of the problems can be solved by other algorithm, one solution cannot be applied to another. dynamic programming technical can be used to solve all of them.  The following relations are in all of the stock problems.

![sell and buy stock](image/sell_and_buy_stock.png)

A release status means possion of no stocks and the hold status means possion of one stock. Release status would be transferred from by selling holding stock or keep current status. Similiarly, hold status would be transferred from by buying if no stock hold or keep current status. In this serial of problems, one can only hold at most one stock. If a customer can buy more stocks, This is simliar, E.g. status of holding 2 stocks and status of holding 3 stocks are similar to release and hold as stated above. 
...
Because one can buy multiple times as long as at most one stock is hold. The maxium profit if buying a stock at ith day should take account of profits gained in previous transactions. dp[i]\[1] = Math.max(dp[i-1]\[1], -prices[i]); in previous problem should be modified to dp[i]\[1] = Math.max(dp[i-1]\[1], dp[i-1]\[0] - prices[i]);
...
Simliar to previous problem, the space complexity can be reduced to constant.
...
There is also a greedy solution to this problem that is buying and selling the stock as long as prices[i-1] < prices[i] because one can purchase as many time as s/he wants. 
...
Apparently, we need a way to repesent transfering from zero transaction to one transaction and from one transaction to two transactions. So, an additional demision is required.
...
The state transformation is similiar to previous image. The transaction is defined at selling a holding stock. So, the dp[i]\[j]\[0] is transfer from rest or from selling stock from previous purcahse with j -1 transactions. The holding is from purchase stock from the release status with j transaction because the transaction is completed only after selling the stock. 

We actually does not need dp[][][i]\[2]\[1] because it cannot be sold and there is a penalty of - prices[i] and dp[][][i]\[0]\[0] is always 0.dp[0]\[1] and dp[0]\[2] are impossible. the should be intialized to a very small value.
...
Space complexity: the size of the addy 2\*3\*N => O(N).
...
This is a variantion of problem II. There is no limit on how many transaction one can perform but no purchase on the day and the next day after selling the stock.

Apparently, we still need a third demsion. It it different from III and IV presenting the transation. In this case, that demsion is to tell if a stock was told on that day.
...
This is A variant of II. One can buy and sell the stock as many times as s/he wants but there is a tansation fee at the time of selling the stock. The optimal substructure is the the same as the II except there is a fee added when dp[i-1]\[1] is tranferred to dp[i]\[0].
