...
| substrings (all consecutive elements starting from or ending by the current ) | O(n^2) | increase the substring |
...
Time Complexity O(2^n) . Space Complexity (log(n))
...
2. DP
   ...
   1.  BF
      Try all possible combinations O(2^n)
