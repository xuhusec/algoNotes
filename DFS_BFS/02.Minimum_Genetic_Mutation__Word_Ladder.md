...
Here we need to clarify the function provided by `visited.add(next)`. The purpose of this condition is to avoid go back to previous layer in BFS.  By knowing this, we can modify here. We need separate the set. One is for all previous levels and another is for current level. The first one would add all the elements in the second one at the end of each iterations. The breaking condition would also changed to allow more than one answer. We can check if the result list contains element. If it does, we found the answer.
...
Space complexity:  The entire paths are saved. if we count characters O(m*n^2^)
